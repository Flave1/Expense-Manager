...
namespace PlugLitePortalHelper.APIServices
{
    ...
    public class RequestTypeSettingServices
    {
        ...
        public static RequestTypeSettingRegRespObj UpdateRequestTypeSetting(EditRequestTypeSettingObj regObj, string username)
        {
            var response = new RequestTypeSettingRegRespObj
            {
                Status = new APIResponseStatus
                {
                    IsSuccessful = false,
            ...
            try
            {
                ...
                var apiResponse = new APIHelper(APIEndpoints.UPDATE_REQUEST_TYPE_SETTINGS_ENDPOINT, username, Method.POST).ProcessAPI<EditRequestTypeSettingObj, RequestTypeSettingRegRespObj>(regObj, out var msg);
                if (msg.Code == 0 && string.IsNullOrEmpty(msg.TechMessage) && string.IsNullOrEmpty(msg.Message))
                {
                    return apiResponse;
                }
        ...
        public static RequestTypeSettingRegRespObj AddRequestTypeSetting(RegRequestTypeSettingObj regObj, string username)
        {
            var response = new RequestTypeSettingRegRespObj
            {
                Status = new APIResponseStatus
            ...
            try
            {
                ...
                var apiResponse = new APIHelper(APIEndpoints.ADD_REQUEST_TYPE_SETTINGS_ENDPOINT, username, Method.POST).ProcessAPI<RegRequestTypeSettingObj, RequestTypeSettingRegRespObj>(regObj, out var msg);
                if (msg.Code == 0 && string.IsNullOrEmpty(msg.TechMessage) && string.IsNullOrEmpty(msg.Message))
                {
                    return apiResponse;
                }
